...
const EditMealPage = () => {
  ...
  const loadMeal = async () => {
    try {
      ...
      if (response.success) {
        ...
        if (!user || meal.createdBy._id !== user._id) {
        ...
        setFormData({
          ...
          nutrition: {
            ...
            glycemicIndex: meal.nutrition?.glycemicIndex || ''
          },
          ingredients: meal.ingredients?.length > 0 ? meal.ingredients : [''],
          allergens: meal.allergens?.length > 0 ? meal.allergens : [''],
          ...
          appliances: meal.appliances?.length > 0 ? meal.appliances : ['']
        })

        // Set current image if exists
        if (meal.image) {
          setCurrentImage(meal.image)
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await mealService.updateMeal(mealId, submitData)
      
      if (response.success) {
        success('Meal updated successfully!')
        navigate(`/dashboard/meals/${mealId}`)
      }
    } catch (err) {
      error(err.message || 'Failed to update meal')
    } finally {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <LoadingSpinner size="lg" message="Loading meal..." />
      </div>
    )
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      >
        <button
          onClick={() => navigate(`/dashboard/meals/${mealId}`)}
      ...
      >
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            ...
            <span>{isSubmitting ? 'Updating...' : 'Update Meal'}</span>
          </button>
        </div>
      </motion.form>
    </div>
